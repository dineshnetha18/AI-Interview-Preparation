...
const NavItems = [
  ...
  {NavItems.map((item, index) => (
    <NavLink
      ...
      className={({ isActive }) =>
        `${isActive
          ? 'text-[#6B6BFA] bg-[#6B6BFA]/10'
          : 'text-white/70 hover:text-white'} 
          flex flex-col hover:shadow-[0_0_20px_#5866F24D] px-3 space-y-0.5 rounded-2xl py-1.5 items-start cursor-pointer transition duration-300`
      }
    >
